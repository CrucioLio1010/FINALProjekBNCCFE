...
submitBtn.addEventListener('click',function(event){
    ...
    if(userNameInput.length<3){
        error1.innerText='Nama harus diisi dan panjang nama harus lebih dari sama dengan 3 karakter';
    }

    else if((!(userEmailInput.includes('@')))||userEmailInput.length==0){
        error2.innerText='Email harus diisi dan mencantumkan @';
    }
    
    else if((!(userPhoneNumberInput.startsWith('08')))||
    userPhoneNumberInput.length==0||
    userPhoneNumberInput.length>14){
        error3.innerText='Nomor telepon harus diisi, digit pertama harus 0, digit kedua harus 8, dan panjang nomor telepon tidak lebih dari 14 digit';
    }
